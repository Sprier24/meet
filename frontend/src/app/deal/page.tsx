...
const formSchema = z.object({
    ...
    status: z.enum(["Discussion", "Demo", "Proposal", "Decided"]),
...
export default function DealPage() {
    ...
    const fetchDeals = async () => {
        try {
            ...
            if (typeof response.data === 'object' && 'data' in response.data) {
                // Response format: { data: [...deals] }
                dealsData = response.data.data;
            } else if (Array.isArray(response.data)) {
                // Response format: [...deals]
            ...
            const dealsWithKeys = dealsData.map((deal: Deal) => ({
                ...deal,
    ...
    const handleEditClick = (deal: Deal) => {
        ...
        form.reset({
            ...
            status: deal.status as "Discussion" | "Demo" | "Proposal" | "Decided",
            date: deal.date ? new Date(deal.date) : undefined,
    ...
    const handleDeleteClick = async (deal: Deal) => {
        if (!window.confirm("Are you sure you want to delete this deal?")) {
            return;
        }

        try {
            const response = await fetch(`http://localhost:8000/api/v1/deal/deleteDeal/${deal._id}`, {
                method: "DELETE",
            });
    ...
    const renderCell = React.useCallback((deal: Deal, columnKey: string) => {
